...
class ArrayMapperFactory
{
    public function __invoke($services)
    {
        if (! $services->has('config')) {
            throw new DomainException('Cannot create DemoLib\ArrayMapper; missing config dependency');
        }

        $config = $services->get('config');
        if (! isset($config['demolib']['array_mapper_path'])) {
            throw new DomainException(sprintf(
                'Cannot create %s; missing demolib.array_mapper_path configuration',
                ArrayMapper::class
            ));
        }

        $path = $config['demolib']['array_mapper_path'];
        if (! file_exists($path)) {
            throw new DomainException(sprintf(
